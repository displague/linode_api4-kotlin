...
import io.swagger.client.models.LinodeConfigHelpers
 ...
 * @param kernel A Kernel ID to boot a Linode with. Defaults to \"linode/latest-64bit\". 
 ...
 * @param virtMode Controls the virtualization mode. Defaults to `paravirt`. * `paravirt` is suitable for most cases. Linodes running in paravirt mode   share some qualities with the host, ultimately making it run faster since   there is less transition between it and the host. * `full_virt` affords more customization, but is slower because 100% of the VM   is virtualized.  
 * @param helpers  
 * @param label The Config's label is for display purposes only.  
 * @param devices  
...
data class LinodeConfig (
    ...
    val virtMode: LinodeConfig.VirtMode? = null,
    val helpers: LinodeConfigHelpers? = null,
    /* The root device to boot. The corresponding disk must be attached.  */
    val rootDevice: kotlin.String? = null
) {

    /**
    * Defines the state of your Linode after booting. Defaults to `default`. 
    * Values: default,single,binbash
    */
    enum class RunLevel(val value: kotlin.String){
    ...
    /**
    * Controls the virtualization mode. Defaults to `paravirt`. * `paravirt` is suitable for most cases. Linodes running in paravirt mode   share some qualities with the host, ultimately making it run faster since   there is less transition between it and the host. * `full_virt` affords more customization, but is slower because 100% of the VM   is virtualized. 
    * Values: paravirt,fullvirt
    */
    enum class VirtMode(val value: kotlin.String){
