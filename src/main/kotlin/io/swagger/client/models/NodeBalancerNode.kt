 ...
 * @param id This node's unique ID. 
 * @param address The private IP Address where this backend can be reached. This _must_ be a private IP address.  
 * @param label The label for this node.  This is for display purposes only.  
 * @param status The current status of this node, based on the configured checks of its NodeBalancer Config.  
 * @param weight Used when picking a backend to serve a request and is not pinned to a single backend yet.  Nodes with a higher weight will receive more traffic.  
 * @param mode The mode this NodeBalancer should use when sending traffic to this backend. * If set to `accept` this backend is accepting traffic. * If set to `reject` this backend will not receive traffic. * If set to `drain` this backend will not receive _new_ traffic, but connections already   pinned to it will continue to be routed to it.  
 * @param configId The NodeBalancer Config ID that this Node belongs to.  
 * @param nodebalancerId The NodeBalancer ID that this Node belongs to. */
data class NodeBalancerNode (
    ...
    val status: NodeBalancerNode.Status? = null,
    ...
    val mode: NodeBalancerNode.Mode? = null,
