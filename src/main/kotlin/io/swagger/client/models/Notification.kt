...
import io.swagger.client.models.NotificationEntity
 ...
 * @param entity  
 * @param label A short description of this Notification.  
 * @param message A human-readable description of the Notification. 
 * @param type The type of Notification this is. 
 * @param severity The severity of this Notification.  This field can be used to decide how prominently to display the Notification, what color to make the display text, etc.  
 * @param &#x60;when&#x60; If this Notification is of an Event that will happen at a fixed, future time, this is when the named action will be taken. For example, if a Linode is to be migrated in response to a Security Advisory, this field will contain the approximate time the Linode will be taken offline for migration.  
 * @param until If this Notification has a duration, this will be the ending time for the Event/action. For example, if there is scheduled maintenance for one of our systems, `until` would be set to the end of the maintenance window. */
data class Notification (
    ...
    val severity: Notification.Severity? = null,
    ...
    val `when`: java.time.LocalDateTime? = null,
    ...
    val until: java.time.LocalDateTime? = null
) {

    /**
    * The type of Notification this is.
    * Values: migrationScheduled,migrationPending,rebootScheduled,outage,paymentDue,ticketImportant,ticketAbuse,notice
    */
